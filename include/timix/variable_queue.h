 ...
 *
 *  @author Tim Shields <tashields@cmu.edu>
 **/

#include <stddef.h>

#define Q_INSERT_HEAD(Q_HEAD, Q_ELEM, LINK_NAME) \
            Q_INSERT_FRONT(Q_HEAD, Q_ELEM, LINK_NAME)

#define Q_INSERT_BACK(Q_HEAD, Q_ELEM, LINK_NAME) \
            Q_INSERT_TAIL(Q_HEAD, Q_ELEM, LINK_NAME)
...
/**
 * @brief Concatinates two queues together
 *
 * Appends elements in HEAD2 to those in HEAD1
 * @param Q_HEAD1 First queue, also the destination
 * @param Q_HEAD2 Second queue to append. Will be empty after
 *          concatination. 
 */
#define Q_CONCAT(Q_HEAD1, Q_HEAD2, LINK_NAME)                       \
        do {                                                        \
            if((Q_HEAD1)->tail == NULL) {                           \
                (Q_HEAD1)->head = (Q_HEAD2)->head;                  \
                (Q_HEAD1)->tail = (Q_HEAD2)->tail;                  \
            } else if((Q_HEAD2)->head != NULL) {                    \
...
#define Q_INSERT_AFTER(Q_HEAD,Q_INQ,Q_TOINSERT,LINK_NAME)                     \
        do {                                                                  \
            if((Q_HEAD)->tail == (Q_INQ)) {                                   \
                (Q_HEAD)->ptr = (Q_HEAD)->tail;                               \
                (Q_HEAD)->tail = (Q_TOINSERT);                                \
            } else {                                                          \
                (Q_HEAD)->ptr = (Q_INQ)->LINK_NAME.next;                      \
                (Q_HEAD)->ptr->LINK_NAME.prev = (Q_TOINSERT);                 \
                (Q_TOINSERT)->LINK_NAME.next = (Q_HEAD)->ptr;                 \
            }                                                                 \
            (Q_HEAD)->ptr->LINK_NAME.next = (Q_TOINSERT);                     \
            (Q_TOINSERT)->LINK_NAME.prev = (Q_HEAD)->ptr;                     \
        } while(0)
...
#define Q_INSERT_BEFORE(Q_HEAD,Q_INQ,Q_TOINSERT,LINK_NAME)                    \
        do {                                                                  \
            if((Q_HEAD)->head == (Q_INQ)) {                                   \
                (Q_HEAD)->ptr = (Q_HEAD)->head;                               \
                (Q_HEAD)->head = (Q_TOINSERT);                                \
            } else {                                                          \
                (Q_HEAD)->ptr = (Q_INQ)->LINK_NAME.prev;                      \
                (Q_HEAD)->ptr->LINK_NAME.next = (Q_TOINSERT);                 \
                (Q_TOINSERT)->LINK_NAME.prev = (Q_HEAD)->ptr;                 \
            }                                                                 \
 ...
 *  is undefined.
 *
 *  The element Q_ELEM will also have it's link re-initialized
 *
...
#define Q_REMOVE(Q_HEAD,Q_ELEM,LINK_NAME)                                   \
        do {                                                                \
            if((Q_HEAD)->head == (Q_ELEM)) {                                \
                (Q_HEAD)->head = (Q_ELEM)->LINK_NAME.next;                  \
            }                                                               \
            if((Q_HEAD)->tail == (Q_ELEM)) {                                \
                (Q_HEAD)->tail = (Q_ELEM)->LINK_NAME.prev;                  \
            }                                                               \
            (Q_HEAD)->ptr = (Q_ELEM)->LINK_NAME.prev;                       \
            if((Q_HEAD)->ptr != NULL)                                       \
                (Q_HEAD)->ptr->LINK_NAME.next = (Q_ELEM)->LINK_NAME.next;   \
            (Q_HEAD)->ptr = (Q_ELEM)->LINK_NAME.next;                       \
            if((Q_HEAD)->ptr != NULL)                                       \
                (Q_HEAD)->ptr->LINK_NAME.prev = (Q_ELEM)->LINK_NAME.prev;   \
